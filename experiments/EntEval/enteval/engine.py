...
from enteval.ed import RareEval, ConllYagoEval, WLNEDEval, EHREval
    ...
    def eval(self, name):
        ...
        if (isinstance(name, list)):
            ...
            for i, x in enumerate(name):
                print("{}: Running an experiment for {}".format(i, x))
                self.results[x] = self.eval(x)
            return self.results
        ...
        if name == 'CAPsame':
            ...
            self.evaluation = WLNEDEval(tpath + '/WLNED/', seed=self.params.seed)
        elif name == 'EHR':
            self.evaluation = EHREval(tpath + '/ehr/', seed=self.params.seed)   #TODO put data here

        self.params.current_task = name
        print('Running do_prepare...')
        self.evaluation.do_prepare(self.params, self.prepare)

        print('Running evaluation.run...')
